...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	AdcRegs.SOCPRICTL.bit.SOCPRIORITY = 1;  // SOC0 is high priority, rest of channels are in round robin mode
	
	AdcRegs.INTSEL1N2.bit.INT2E = 1;		// Enabled ADCINT2
	AdcRegs.INTSEL1N2.bit.INT2CONT = 1;		// Enable ADCINT2 Continuous mode,the ADCINTy pulses are generated whenever an EOC pulse is generated irrespective if the flag bit is cleared or not. 
	AdcRegs.INTSEL1N2.bit.INT2SEL = 0;  	// setup EOC0 to trigger ADCINT2 to CLA Task2

	AdcRegs.INTSEL7N8.bit.INT7E = 1;		// Enabled ADCINT7
	AdcRegs.INTSEL7N8.bit.INT7CONT = 1;		// Enable ADCINT7 Continuous mode,the ADCINTy pulses are generated whenever an EOC pulse is generated irrespective if the flag bit is cleared or not. 
	AdcRegs.INTSEL7N8.bit.INT7SEL = 2;  	// setup EOC2 to trigger ADCINT7 to CLA Task7

	AdcRegs.ADCSOC0CTL.bit.CHSEL = 3;  		// set SOC0 channel select to ADCINA3 ISen current
	AdcRegs.ADCSOC1CTL.bit.CHSEL = 2;  		// set SOC1 channel select to ADCINA2 Temp
	AdcRegs.ADCSOC2CTL.bit.CHSEL = 1;  		// set SOC2 channel select to ADCINA1 Ign

	AdcRegs.ADCSOC0CTL.bit.TRIGSEL = ePWM3ADCSOCA;  	// set SOC0 start trigger on EPWM3A
	AdcRegs.ADCSOC1CTL.bit.TRIGSEL = ePWM2ADCSOCA;  	// set SOC1 start trigger on EPWM2A
	AdcRegs.ADCSOC2CTL.bit.TRIGSEL = ePWM2ADCSOCA;  	// set SOC2 start trigger on EPWM2A
	
	AdcRegs.ADCSOC0CTL.bit.ACQPS = 6;		// set SOC1 S/H Window to 7 ADC Clock Cycles, (6 ACQPS plus 1, 60MHz, total time 333.33ns)
	AdcRegs.ADCSOC1CTL.bit.ACQPS = 6;		// set SOC0 S/H Window to 7 ADC Clock Cycles, (6 ACQPS plus 1, 60MHz, total time 333.33ns)
	AdcRegs.ADCSOC2CTL.bit.ACQPS = 6;		// set SOC1 S/H Window to 7 ADC Clock Cycles, (6 ACQPS plus 1, 60MHz, total time 333.33ns)
	EDIS;
