...
{       
    ...
    if((uint32_T)(System_counter - CpuTimer2Regs.TIM.all) > WD_Service_Time)
	{
	    EALLOW;
	    SysCtrlRegs.WDKEY = 0x0055;
	    SysCtrlRegs.WDKEY = 0x00AA;
	    EDIS;
		System_counter = CpuTimer2Regs.TIM.all;
	}
}

#pragma CODE_SECTION(API_Flash_Erase,"ramfuncs");

uint16_T API_Flash_Erase(uint16_T sector)
{
	return (Flash_Erase(sector, &FlashStatus));
}

#pragma CODE_SECTION(API_Flash_Program,"ramfuncs");

uint16_T API_Flash_Program(uint16_T *FlashAddr, uint16_T *BufAddr, uint32_T Length)
{
	return (Flash_Program(FlashAddr,BufAddr,Length,&FlashStatus));
}

#pragma CODE_SECTION(API_Flash_Verify,"ramfuncs");

uint16_T API_Flash_Verify(uint16_T *FlashAddr, uint16_T *BufAddr, uint16_T Length)
{
	return (Flash_Verify(FlashAddr,BufAddr,Length,&FlashStatus));
}
